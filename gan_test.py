...
def main():
    ...
    image = tf.Variable(tf.zeros((299, 299, 3)), name="variable_input")

    logits, probs = get_inception_v3_model(image, reuse=False)

    restore_imagenet_model_locally(sess)
    imagenet_labels = get_imagenet_labels()

    img = open_image(url_elephant)
    img_class = None

    x_hat = image # trainable adversarial input
    x = tf.placeholder(tf.float32, (299, 299, 3), name='x')
    ...
    projected = tf.clip_by_value(tf.clip_by_value(x_hat, below, above, name="clip-epsilon"), 0, 1, name="clip-0-1")
    ...
    p_m =classify(adv, image, sess, imagenet_labels, probs, correct_class=img_class, target_class=demo_target)
    display_diff(adv, img)
